...
(function($) {
    ...
    $.fn.fileinput = function(replacement) {
        var selector = this;
        var browse_text = "Browse...";
        if (typeof(browse_translation) != "undefined") {
            browse_text = browse_translation;
        }
        var replacementHtml = "<button class=\"fileinput\">" + browse_text + "</button>";
        if (replacement) {
        ...
        selector.each(function() {
            ...
            element.prev().addClass(language)

            element.change(function(){
            	var filename = $(this).val().replace(/C:\\fakepath\\/i, '');
            	element.prev(".fileinput").find('span').text(filename);
            });
